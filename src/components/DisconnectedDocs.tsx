...
import { FileText, ShieldQuestion, Wallet, Leaf, FileCheck2, Scale } from "lucide-react";
    ...
    <div className="relative h-[380px] md:h-[440px]">
      ...
      <div className="relative grid grid-cols-3 sm:grid-cols-3 gap-4 md:gap-5 h-full items-center">
        {docs.map((d, i) => (
          ...
          >
            <span className="absolute -top-1.5 -right-1.5 h-5 w-5 rounded-full bg-destructive text-destructive-foreground text-[10px] font-bold flex items-center justify-center shadow-sm">
              !
            </span>
            <div
              className={`h-9 w-9 rounded-md bg-secondary flex items-center justify-center ${d.tone}`}
            >
              <d.icon className="h-4.5 w-4.5" />
            </div>
            <div className="mt-3 text-[13px] font-semibold text-primary leading-tight">
              {d.label}
            </div>
